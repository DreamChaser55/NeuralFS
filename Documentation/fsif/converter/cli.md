...
## Advanced Validation
- The Advanced SEXP Validator runs automatically. It performs a deep semantic check on all FSIF SEXP-bearing fields — event and goal formulas, ship/wing `arrival_condition`, `departure_condition`, and `initial_orders`, and debriefing `display_condition` — using FSO engine logic (type checking, return types, reference validation).
