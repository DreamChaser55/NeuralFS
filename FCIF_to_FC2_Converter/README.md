...
- The converter verifies loadout validity for every mission whose `.fsif` file is successfully parsed and will reject the `.fcif` file if a ship or weapon is used by the player without being in `starting_loadout` or granted by an `allow-ship`/`allow-weapon` SEXP in a prior mission. If the `.fsif` file for a given mission is missing or unreadable, that mission is skipped with a warning.
