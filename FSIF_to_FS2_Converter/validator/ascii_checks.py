...
class AsciiChecksMixin:
    ...
    def validate_ascii_text_fields(self):
        ...
        if env.nebula:
            ...
            self._validate_ascii_text_list('environment.nebula.cloud_sprites', env.nebula.poofs)

        if env.asteroid_field:
            self._validate_ascii_text_list('environment.asteroid_field.object_variants', env.asteroid_field.debris_types)
            self._validate_ascii_text_list('environment.asteroid_field.target_ships', env.asteroid_field.targets)
        ...
        self._validate_ascii_text_list('player_setup.additional_weapons', setup.extra_weapons)
        ...
        for i, ship in enumerate(self.mission.ships):
            ...
            self._validate_ascii_text(f'{prefix}.arrival_method', ship.arrival_location)
            self._validate_ascii_text(f'{prefix}.arrival_anchor', ship.arrival_anchor)
            self._validate_ascii_text(f'{prefix}.arrival_condition', ship.arrival_cue)
            self._validate_ascii_text(f'{prefix}.departure_method', ship.departure_location)
            self._validate_ascii_text(f'{prefix}.departure_anchor', ship.departure_anchor)
            self._validate_ascii_text(f'{prefix}.departure_condition', ship.departure_cue)
            self._validate_ascii_text_list(f'{prefix}.flags', ship.flags)
            self._validate_ascii_text(f'{prefix}.initial_orders', ship.ai_goals)
        ...
        for i, wing in enumerate(self.mission.wings):
            ...
            self._validate_ascii_text(f'{prefix}.arrival_method', wing.arrival_location)
            self._validate_ascii_text(f'{prefix}.arrival_anchor', wing.arrival_anchor)
            self._validate_ascii_text(f'{prefix}.arrival_condition', wing.arrival_cue)
            self._validate_ascii_text(f'{prefix}.departure_method', wing.departure_location)
            self._validate_ascii_text(f'{prefix}.departure_anchor', wing.departure_anchor)
            self._validate_ascii_text(f'{prefix}.departure_condition', wing.departure_cue)
            self._validate_ascii_text_list(f'{prefix}.flags', wing.flags)
            self._validate_ascii_text(f'{prefix}.initial_orders', wing.ai_goals)
        ...
        for i, event in enumerate(self.mission.events):
            ...
            self._validate_xstr_text(f'{prefix}.hud_directive_text', event.directive_text)
        ...
        for i, goal in enumerate(self.mission.goals):
            ...
            self._validate_xstr_text(f'{prefix}.objective_text', goal.message)
        ...
        for i, message in enumerate(self.mission.messages):
            ...
            self._validate_xstr_text(f'{prefix}.text', message.message)
        ...
        for i, stage in enumerate(self.mission.briefing.stages):
            ...
            for j, icon in enumerate(stage.icons):
                ...
                self._validate_ascii_text(f'{icon_prefix}.icon_type', icon.type)
                self._validate_ascii_text(f'{icon_prefix}.team', icon.team)
                self._validate_ascii_text(f'{icon_prefix}.display_class', icon.ship_class)
        ...
        for i, stage in enumerate(self.mission.debriefing.stages):
            ...
            self._validate_xstr_text(f'{prefix}.text', stage.text)
            self._validate_ascii_text(f'{prefix}.display_condition', stage.condition)
        ...
        for i, reinforcement in enumerate(self.mission.reinforcements):
            ...
            self._validate_ascii_text_list(f'{prefix}.unavailable_messages', reinforcement.no_messages)
            self._validate_ascii_text_list(f'{prefix}.available_messages', reinforcement.yes_messages)
