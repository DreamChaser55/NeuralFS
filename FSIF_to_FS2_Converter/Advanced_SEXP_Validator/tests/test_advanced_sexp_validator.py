...
from advanced_sexp_validator import SexpParser, SexpValidator, MissionContext, OPR_NULL, OPR_BOOL, OPR_NUMBER, OPR_AI_GOAL
import fs_data
...
class TestAdvancedSexpValidator(unittest.TestCase):
    def setUp(self):
        ...
        self.ctx.variables["hull_strength"] = OPR_NUMBER
    ...
    def validate_string(self, sexp_str, expected_type=OPR_NULL):
    ...
    def test_valid_basic(self):
        errors = self.validate_string('(when (true) (do-nothing))', OPR_NULL)
        self.assertEqual(errors, [])

    def test_valid_weapon_unlock(self):
        errors = self.validate_string('(allow-weapon "Prometheus")', OPR_NULL)
        self.assertEqual(errors, [])

    def test_valid_subsystem(self):
        errors = self.validate_string('(when (is-subsystem-destroyed-delay "Alpha 1" "navigation" 0) (do-nothing))', OPR_NULL)
        self.assertEqual(errors, [])

    def test_invalid_weapon_name(self):
        errors = self.validate_string('(allow-weapon "SuperMegaLaser")', OPR_NULL)
    ...
    def test_invalid_subsystem_name(self):
        errors = self.validate_string('(when (is-subsystem-destroyed-delay "Alpha 1" "flux_capacitor" 0) (do-nothing))', OPR_NULL)
    ...
    def test_invalid_ship_class(self):
        errors = self.validate_string('(allow-ship "StarDestroyer")', OPR_NULL)
        self.assertTrue(any("Invalid Ship Class" in e for e in errors))

    def test_invalid_iff(self):
        errors = self.validate_string('(change-iff "Borg" "Alpha 1")', OPR_NULL)
        self.assertTrue(any("Invalid Team" in e for e in errors))

    def test_invalid_ship_name(self):
        errors = self.validate_string('(is-destroyed-delay 0 "NonExistentShip")', OPR_BOOL)
    ...
    def test_valid_ship_flag(self):
        errors = self.validate_string('(alter-ship-flag "cargo-known" (true) (true) "Alpha 1")', OPR_NULL)
        self.assertEqual(errors, [])

    def test_invalid_ship_flag(self):
        errors = self.validate_string('(alter-ship-flag "invalid-flag" (true) (true) "Alpha 1")', OPR_NULL)
        self.assertTrue(any("Invalid Ship Flag" in e for e in errors))

    def test_valid_music(self):
        errors = self.validate_string('(change-soundtrack "1: Genesis")', OPR_NULL)
        self.assertEqual(errors, [])

    def test_invalid_music(self):
        errors = self.validate_string('(change-soundtrack "Invalid Track")', OPR_NULL)
    ...
    def test_who_from_valid_ship(self):
        ...
        errors = self.validate_string('(send-message "Alpha 1" "High" "Message 1")', OPR_NULL)
        self.assertEqual(errors, [])

    def test_who_from_valid_wing(self):
        errors = self.validate_string('(send-message "Alpha" "High" "Message 1")', OPR_NULL)
        self.assertEqual(errors, [])

    def test_who_from_valid_special(self):
        errors = self.validate_string('(send-message "#Command" "High" "Message 1")', OPR_NULL)
        self.assertEqual(errors, [])
        errors = self.validate_string('(send-message "<any wingman>" "High" "Message 1")', OPR_NULL)
        self.assertEqual(errors, [])

    def test_who_from_invalid_special(self):
        errors = self.validate_string('(send-message "#InvalidSource" "High" "Message 1")', OPR_NULL)
        self.assertTrue(
        ...
        errors = self.validate_string('(send-message "<any nobody>" "High" "Message 1")', OPR_NULL)
        self.assertTrue(
            any("Invalid Message Sender" in e for e in errors) or
            any("Invalid Special Message Sender" in e for e in errors),
            f"Errors: {errors}"
    ...
    def test_who_from_invalid_generic(self):
        errors = self.validate_string('(send-message "RandomString" "High" "Message 1")', OPR_NULL)
        self.assertTrue(any("Invalid Message Sender" in e for e in errors), f"Errors: {errors}")

    def test_ship_invalid_wildcard(self):
        # ai-chase returns OPR_AI_GOAL
        errors = self.validate_string('(ai-chase "<any friendly>" 0)', OPR_AI_GOAL)
        self.assertEqual(errors, [])

        errors = self.validate_string('(ai-chase "<invalid>" 0)', OPR_AI_GOAL)
    ...
    def test_valid_point_waypoint(self):
        ...
        errors = self.validate_string('(ai-waypoints "Path1" 89)', OPR_AI_GOAL)
        self.assertEqual(errors, [])

    def test_valid_point_specific(self):
        self.ctx.waypoints.add("Path1")
        # distance can use OPF_SHIP_WING_POINT
        errors = self.validate_string('(distance "Alpha 1" "Path1:1")', OPR_NUMBER)
        self.assertEqual(errors, [])

    def test_invalid_point(self):
        # distance can use OPF_SHIP_WING_POINT
        errors = self.validate_string('(distance "Alpha 1" "NonExistentPath:1")', OPR_NUMBER)
    ...
    def test_valid_ship_point(self):
        ...
        errors = self.validate_string('(is-facing "Alpha 1" "Path1:1" 0)', OPR_BOOL)
        self.assertEqual(errors, [])
        errors = self.validate_string('(is-facing "Alpha 1" "Alpha 1" 0)', OPR_BOOL)
        self.assertEqual(errors, [])

    def test_invalid_ship_point(self):
        # is-facing arg 1 uses OPF_SHIP_POINT
        errors = self.validate_string('(is-facing "Alpha 1" "InvalidTarget" 0)', OPR_BOOL)
